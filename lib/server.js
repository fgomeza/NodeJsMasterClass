    ...
    req.on('end', function() {
        ...
        try {
            chosenHandler(data).catch(e => e).then(payload => {
                server.processHandlersResponse(res, method, trimmedPath, payload);
            });
        } catch(e) {
            debug(e);
            server.processHandlersResponse(res, method, trimmedPath, {'Error': 'An unknown error has occured', 'statusCode': 500, 'contentType': 'json'});
        }
...
server.processHandlersResponse = function (res, method, trimmedPath, payload) {
