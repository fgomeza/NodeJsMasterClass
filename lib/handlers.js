...
handlers.index = function(data) {
    return new Promise(function (resolve, reject) {
        ...
        if (data.method == 'GET') {
            ...
            .then((template) => resolve({ contentType: 'html', statusCode: 200, body: template }))
        } else {
    ...
    if (phone && password) {
        ...
        .then((userData) => {
            ...
            if (hashedPassword == userData.hashedPassword) {
                ...
                var tokenObj = {
                    'phone': phone,
                    'tokenId': helpers.createRandomString(TOKEN_LENGTH),
                    'expires': Date.now() + 1000 * 60 * 60
                };

                // Store the user
                return _data.create('tokens', tokenObj.tokenId, tokenObj)
                    .catch(err => {
                        console.error(err);
                        return errorResponse(500, 'Could not create the new user');
                    }).then(() => validResponse(200, tokenObj));
            } else {
                console.log('Francisco C');
